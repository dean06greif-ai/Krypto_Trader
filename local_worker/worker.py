...
class Api:
    ...
    async def post(self, path: str, payload: dict, gzip_body: bool = False):
        ...
        async with s.post(url, data=raw, headers=headers) as r:
            text = await r.text()
            if r.status >= 400:
...
class Worker:
    ...
    async def _execute(self, item: dict):
        ...
        try:
            ...
            if kind in HANDLERS:
                ...
                task = asyncio.create_task(HANDLERS[kind](job_id, args))
                ...
                task = asyncio.create_task(_run_data_job(job_id, kind, payload,
                                                         self.local_jobs))
                watcher = asyncio.create_task(self._progress_loop(job_id, job, task))
                out = await task
                watcher.cancel()
    ...
    async def _poll(self, want: bool) -> dict:
        ...
        return await self.api.post("/api/worker/poll", body)
