...
def _ideal_payload(candles: List[Dict], labels: List, model: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        return {"segments": _segments_payload(candles, ideal),
                "agreement": eng.agreement_with_ideal(labels, ideal),
                "lookahead": True}
    ...
    try:
        ...
        if engine == "v2":
            ...
            engine_config = {**engine_config,
                             ...
                             "min_hold_days": engine_config.get("min_hold_days",
                                                                min_hold_days)}
        ...
        if scope in ("both", "combined"):
            ...
            if model:
                ...
                for sym, candles in histories.items():
                    ...
                    per_symbol[sym] = {
                        "segments": _segments_payload(candles, labels),
        ...
        if scope in ("both", "per_coin"):
            for i, (sym, candles) in enumerate(histories.items()):
                ...
                per_coin[sym] = {"model": model_s, "segments": segs,
                                 ...
                                 "validation": _validation_payload(candles, labels,
                                                                   model_s),
                                 "current": _current_payload(candles, model_s, timeframe,
                                                             conf_min, min_hold_days),
                                 "ideal": (_ideal_payload(candles, labels, model_s)
                                           if with_ideal else None)}
