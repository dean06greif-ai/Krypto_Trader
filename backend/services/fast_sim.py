...
def build_custom_provider(strategy, fs: FastSeries, settings: Dict,
                          symbol: str = None) -> Callable[[int], Optional[Dict]]:
    """Custom-Provider inkl. Parameter-Overrides aus den Settings
    (strategy_params/coin_params) – identisch zum Referenzpfad (analyze).
    Ohne Overrides entspricht das exakt build_signal_provider(definition)."""
    definition = strategy.effective_definition(
        strategy.get_params(settings or {}, symbol))
