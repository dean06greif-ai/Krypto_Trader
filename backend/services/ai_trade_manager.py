...
ACTIONS = ("hold", "close", "partial_close", "adjust_sl", "adjust_tp",
           "add_margin", "remove_margin", "set_leverage")
...
TRADE_MANAGER_SYSTEM = (
    ...
    '"new_trades": [{"symbol": "BTCUSDT", "side": "LONG", "sl_pct": 0.8, "tp1_pct": 1.2, '
    '"tpf_pct": 2.0, "leverage": 10, "capital_pct": 50, "confidence": 70, "reason": "..."}], '
...
def check_limits(trade: Dict, action: str, value: Optional[float],
                 settings: Dict, now_ts: Optional[float] = None) -> Tuple[bool, str]:
    """Harte Schutzregeln vor jeder KI-Aktion (rein, testbar)."""
    if action not in ACTIONS:
        return False, f"Unbekannte Aktion '{action}'"
    if action == "hold":
        return True, ""
    if action in ("add_margin", "remove_margin", "set_leverage") \
    ...
    if action == "set_leverage":
        ...
        if lev < 1 or lev > float(settings.get("max_leverage", 50)):
            return False, f"Hebel {lev}x außerhalb des erlaubten Bereichs " \
                          f"(1-{settings.get('max_leverage', 50)})"
    if action in ("add_margin", "remove_margin"):
        try:
        ...
        if action == "remove_margin" and amount >= margin:
...
class AITradeManager:
    ...
    async def update_settings(self, updates: Dict) -> Dict:
        ...
        for key, lo, hi in (("interval_min", 1, 120), ("max_actions_per_trade", 1, 50),
                            ("cooldown_min", 0, 120), ("max_leverage", 1, 125),
    ...
    async def _audit(self, trade: Dict, action: str, value, reason: str,
        ...
        if ok:
            ...
            # Fehlgeschlagene Aktionen (z.B. Börse lehnt den Close ab) sichtbar machen
            try:
                await self.db.ai_chat.insert_one({
                    "id": str(uuid.uuid4()), "role": "trade",
                    "text": f"{action.upper()} auf {trade.get('symbol')} "
    ...
    async def open_trade(self, spec: Dict, source: str = "ki") -> Dict:
        ...
        if not ok:
            return {"status": "rejected",
                    "detail": "Signal wurde von der Trade-Pipeline abgelehnt "
    ...
    async def review(self, manual: bool = False) -> Dict:
        ...
        try:
            trades = await self.db.auto_trades.find({"status": "open"}) \
            ...
            opened = []
            if self.settings.get("allow_open", True):
