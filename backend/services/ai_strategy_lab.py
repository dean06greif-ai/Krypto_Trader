...
TESTING_NOTE = (
    ...
    + supported_summary() + " Solche Kandidaten erscheinen automatisch im Backtester/"
...
ASSIST_SYSTEM = (
    ...
    "WICHTIG für rule_definition: " + supported_summary()
)
...
class StrategyLab:
    ...
    async def assist(self, spec: Dict, cid: Optional[str] = None,
        ...
        rd_errors: List[str] = []
        if rd is not None:
            # Gegen die unterstützte Engine prüfen/normalisieren – verhindert
            # "0 Trades überall", weil unbekannte Indikatoren nie feuern.
            rd, rd_errors = normalize_rule_definition(rd)
        if not valid_rule_definition(rd):
            rd = None
        note = str(data.get("backtest_note", ""))[:400]
        if rd_errors:
            note = (note + " | Regel-Prüfung: " + "; ".join(rd_errors[:4]))[:500]
        out = {
    ...
    async def register_for_testing(self, cid: str) -> Dict:
        ...
        if isinstance(definition, dict):
            # Normalisieren + validieren, damit Backtester/Optimizer die Regeln
            # wirklich auswerten können (statt still 0 Trades zu liefern)
            norm, norm_errors = normalize_rule_definition(definition)
            if norm is None:
                return {"status": "not_testable",
                        "detail": "Regel-Definition nicht backtestbar: "
                                  + "; ".join(norm_errors[:4])}
            if norm_errors:
                logger.info(f"Kandidat {cid}: Regeln normalisiert "
                            f"({'; '.join(norm_errors[:3])})")
            definition = norm
