...
ASSIST_SYSTEM = (
    "Du bist der Strategie-Assistent des 'KI Traders' einer Krypto-Daytrading-Plattform. "
    ...
    '"suggestions": ["konkreter Vorschlag 1", "..."], '
...
class StrategyLab:
    ...
    async def assist(self, spec: Dict, cid: Optional[str] = None,
        ...
        prompt = (
            ...
            f"Name: {name}\n"
            f"Coins: {', '.join(symbols) or 'alle beobachteten Coins'}\n"
            f"Idee/Beschreibung: {thesis or '(leer)'}\n"
            f"Regeln (Prosa): {rules_text or '(keine)'}\n\n"
            "Gib jetzt Feedback, Verbesserungen und – falls machbar – die "
            "maschinenlesbare rule_definition."
        )
        try:
            text, provider, model = await self.engine.generate_for_role(
                "chat", prompt, ASSIST_SYSTEM, temperature=0.3)
