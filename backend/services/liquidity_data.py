...
def _model_clusters(price: float, oi_usd: Optional[float]) -> Dict:
    ...
    for lev, base_strength in LEVERAGE_TIERS:
        ...
        above.append({"price": short_liq, "est_leverage": f"{lev}x",
                      "strength": strength, "dist_pct": round(dist * 100, 2)})
    return {"below_price": below, "above_price": above}
...
# --------------------------------------------------------------------------- #
class _LiqBuffer:
    """Ring buffer of (ts_sec, side, usd) per symbol. side: 'long'|'short' (position liquidated)."""
    def __init__(self):
        self._buf: Dict[str, List[tuple]] = {}

    def add(self, symbol: str, side: str, usd: float):
        if not usd or usd <= 0:
            return
        now = time.time()
        b = self._buf.setdefault(symbol.upper(), [])
        b.append((now, side, usd))
    ...
    def window(self, symbol: str, seconds: int = LIQ_WINDOW_SEC) -> Dict:
        ...
        cnt = 0
        for ts, side, usd in self._buf.get(symbol.upper(), []):
            if ts < cutoff:
                continue
            cnt += 1
            if side == "long":
        ...
        async with s.ws_connect(url, heartbeat=30, timeout=20) as ws:
            async for msg in ws:
                ...
                try:
                    ...
                    _liq_buf.add(sym, side, price * qty)
                except Exception:
                    continue
                ...
                try:
                    ...
                    for row in payload.get("data", []) or []:
                        ...
                        for d in row.get("details", []) or []:
                            ...
                            _liq_buf.add(sym, side, px * sz * ct)
                except Exception:
                    continue
