...
class AutoTradeManager:
    ...
    async def close_live_position(self, t: Dict, qty: float, full: bool = True) -> Dict:
        ...
        for attempt in (1, 2):
            ...
            if ok:
                ...
                verified = await self._verify_flat(t)
                if verified["ok"]:
    ...
    async def _verify_flat(self, t: Dict) -> Dict:
        """Prüft an der Börse nach, ob die Position wirklich weg ist."""
        remaining = None
        for _ in range(3):
    ...
    async def sync_live_levels(self, t: Dict, sl: Optional[float] = None,
        ...
        erst wenn das scheitert, wird eine neue TP/SL-Order gesetzt."""
        if t.get("mode") != "live" or not self.client.configured():
            return {"ok": True, "detail": "kein Live-Trade"}
        ...
        if existing:
            try:
                res = await self.client.modify_position_tp_sl(
                    t["symbol"], existing, tp_price=tp, sl_price=sl, side=t["side"])
        ...
        try:
            res = await self.client.place_position_tp_sl(
                t["symbol"], pid, t["side"],
                tp_price=tp, tp_qty=qty_rem if tp else None,
                sl_price=sl, sl_qty=qty_rem if sl else None)
        except Exception as e:
