        ...
        try:
            ...
            for l in (data.get("lessons") or [])[:max_lessons]:
                ...
                if key in locked_titles:
                    skipped.append(f"{title}: vom Trader festgelegt (unveränderlich)")
                    continue
                ok_master, why = master_prompt.check_lesson(title, detail)
                if not ok_master:
                    skipped.append(f"{title}: {why}")
                    continue
                if not gate_new.get("validated") and key not in old_by_title:
                    skipped.append(f"{title}: {gate_new.get('reason')}")
                    continue
            ...
            if removed and not gate_removal.get("validated"):
                skipped.append(f"Verwerfen von {len(removed)} Lektion(en): "
                               f"{gate_removal.get('reason')}")
                removed = []
            if skipped:
