...
def merge_lessons(old: List[Dict], new: List[Dict], removed: List[str],
    ...
    """Neue Lektionen mit dem bestehenden Wissensstand zusammenführen.

    Bugfix: Vorher wurde die Lektionsliste bei jedem Lernlauf komplett durch die
    LLM-Antwort ERSETZT. Da ein Modell pro Lauf typischerweise nur 3-5 Lektionen
    zurückgibt, blieben trotz `max_lessons = 50` dauerhaft nur ~5 gespeichert.
    Jetzt bleiben bereits gelernte Lektionen erhalten, solange die KI sie nicht
    ausdrücklich verwirft; erst bei Überlauf werden die schwächsten/ältesten
    entfernt.
    """
    drop = {str(t).strip().lower() for t in (removed or []) if str(t).strip()}
    merged: List[Dict] = []
    seen = set()
    for lesson in list(new) + list(old or []):
        key = str(lesson.get("title", "")).strip().lower()
        if not key or key in seen or key in drop:
            continue
        seen.add(key)
        merged.append(lesson)
    merged.sort(key=lambda l: (int(l.get("weight", 2) or 2),
                               str(l.get("updated_at", ""))), reverse=True)
    return merged[:max(1, int(max_lessons))]
...
class AILearning:
    ...
    async def lessons_text(self) -> str:
        ...
        ordered = sorted(lessons, key=lambda l: int(l.get("weight", 2)), reverse=True)
        out = []
        for i, l in enumerate(ordered):
            w = int(l.get("weight", 2))
            label = ai_providers.WEIGHT_LABELS.get(w, "mittel")
            out.append(f"{i + 1}. [Gewicht: {label}] {l.get('title')}: {l.get('detail')}")
        return "\n".join(out)
        ...
        try:
            ...
            for l in (data.get("lessons") or [])[:max_lessons]:
                if isinstance(l, dict) and l.get("title"):
                    ...
                    # Gewichtung: Lektionen stärkerer Modelle behalten ihr hohes
                    # Gewicht, auch wenn ein schwächeres Modell sie später bestätigt.
                    prev = old_by_title.get(title.strip().lower())
                    weight = max(run_weight, int(prev.get("weight", 0))) if prev else run_weight
                    fresh.append({"title": title,
                                  ...
                                  "updated_at": _now_iso(),
                                  "confirmations": int((prev or {}).get("confirmations", 0)) + 1})
            ...
            return {"status": "ok", "lessons": len(lessons), "new_lessons": len(fresh),
                    "removed_lessons": len(removed), "assessment": assessment,
