...
SUPERVISOR_SYSTEM = (
    ...
    "dein KI-Team. Du bekommst pro Rolle: eingesetztes Modell, letzte Aktivität, Fehler, "
    "Kennzahlen und STICHPROBEN echter Ausgaben. Prüfe kritisch und datenbasiert:\n"
    "- Arbeitet die Rolle überhaupt (Aktivität, Fehler, Rate-Limits)?\n"
    "- Sind die Stichproben inhaltlich brauchbar, konkret und widerspruchsfrei?\n"
    ...
    '"recommendations": ["konkrete Empfehlung an den Trader"]}'
)
...
def evidence_text(evidence: Dict) -> str:
    ...
    for role in SUPERVISED_ROLES:
        ...
        lines.append(f"  Modell: {_short(ev.get('model'), 80)} "
                     f"(Fallback: {_short(ev.get('fallback_model'), 80)}) · "
