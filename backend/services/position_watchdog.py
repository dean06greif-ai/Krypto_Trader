...
class PositionWatchdog:
    ...
    async def update_settings(self, updates: Dict) -> Dict:
        for key in ("enabled", "emergency_close", "adopt_unknown"):
            if key in updates:
    ...
    async def run_loop(self):
        ...
        while True:
            try:
                if self.settings.get("enabled", True) and self.client \
    ...
    async def check(self) -> Dict:
        ...
        for pos in positions:
            ...
            try:
                await self._check_position(pos, status)
            except Exception as e:
    ...
    async def _check_position(self, pos: Dict, status: Dict):
        ...
        # nicht übernehmen und keinen SL managen, sondern versuchen zu schließen.
        try:
        ...
        local = await self.db.auto_trades.find_one(
            {"status": "open", "mode": "live", "symbol": internal, "side": pos["side"]})
    ...
    async def _adopt(self, internal: str, pos: Dict) -> Optional[Dict]:
        ...
        trade = {
            ...
            "strategy_id": "external", "strategy_name": "Extern (Watchdog)",
        ...
        await self._notify(
            f"👁️ *WATCHDOG*\n{internal} {pos['side']}: Börsen-Position ohne lokalen "
            f"Trade entdeckt und übernommen (Menge {pos['qty']}, Entry `{entry}`). "
            f"Sie ist jetzt auf der Website sichtbar.")
        return trade
