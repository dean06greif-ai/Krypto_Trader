...
class AIEngine:
    ...
    async def _liquidity_block(self) -> str:
        ...
        for sym in syms:
            ...
            lines.append(
                ...
                f"({b.get('oi_trend')})")
            cl = b.get("liq_clusters") or {}
            below = ", ".join(f"{c.get('price')} ({c.get('est_leverage')}, {c.get('strength')})"
                              for c in (cl.get("below_price") or [])[:3]) or "-"
            above = ", ".join(f"{c.get('price')} ({c.get('est_leverage')}, {c.get('strength')})"
                              for c in (cl.get("above_price") or [])[:3]) or "-"
            lines.append(f"  LIQUIDATIONS-CLUSTER {sym}: unter Preis (Longs) [{below}] | "
                         f"über Preis (Shorts) [{above}]")
    ...
    async def _emit_signal(self, dec: Dict) -> bool:
        ...
        if is_swing:
            ...
            # und harter Hebel-Deckel.
            signal["timeframe"] = "swing"
            signal["ai_leverage"] = float(self.config.get("swing_max_leverage", 8) or 8)
