...
DEFAULT_AI_CONFIG = {
    ...
    # crv_max = 0 -> keine Obergrenze (bisheriges Verhalten).
    "crv_min": 1.2,
    "crv_max": 0,
...
ANALYSIS_SYSTEM = (
    ...
    "suche den dazu passenden Edge. Bei HOLD nenne im reasoning kurz den konkreten Grund "
...
class AIEngine:
    ...
    async def run_analysis(self, manual: bool = False) -> Dict:
        ...
        try:
            ...
            for g_label, g_syms in groups:
                ...
                try:
                    sys_prompt = (ANALYSIS_SYSTEM_LEAN if self.config.get("lean_prompt", True)
    ...
    async def _diversification_gate(self, sym: str, dec: Dict) -> tuple:
    ...
    async def _emit_signal(self, dec: Dict) -> bool:
        ...
        if allowed:
            ...
            allowed, why = await self._diversification_gate(sym, dec)
        if not allowed:
