...
async def _regime_for(db, symbol: str, trade: Dict) -> Optional[str]:
    """Markt-Regime zum Trade – aus dem Markt-Beobachter (live) oder den Snapshots."""
    try:
...
async def ensure_backfill(db) -> int:
    """Einmalig: bereits geschlossene KI-Trades rückwirkend bewerten
    (Re-Check max. alle 10 Minuten, solange noch keine Rewards existieren)."""
    global _backfill_ts
    import time as _t
    ...
    try:
        if await db.ai_rewards.count_documents({}) > 0:
            return 0
        st = await db.settings.find_one({"_id": "ai_rewards_state"}) or {}
        if st.get("cleared_at"):
            return 0  # Trader hat die Daten bewusst gelöscht -> kein Auto-Backfill
...
async def clear(db) -> int:
    """Alle Belohnungsdaten löschen (Backfill bleibt danach aus, neue Trades
    werden weiterhin normal bewertet)."""
