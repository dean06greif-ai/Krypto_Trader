...
def _validate_clear_request(body: Dict):
    """Gemeinsame Validierung + Filter-Aufbau für Clear und Clear-Preview.
    range: 'hour' | '24h' | '7d' | '4w' | 'all'
    scope: 'all' | 'coin' (symbol) | 'coin_strategy' (symbol+strategy_id)
           | 'strategy' (strategy_id über ALLE Coins)"""
    ...
    if scope in ("coin_strategy", "strategy") and not strategy_id:
        raise HTTPException(status_code=400,
                            detail="strategy_id erforderlich für Strategie-Scope")
    ...
    if scope == "coin":
        ...
        scope_filter = {"strategy_id": strategy_id}
    ...
    return rng, scope, sig_filter, trade_filter, cutoff


@router.post("/api/analytics/clear/preview")
async def clear_analytics_preview(body: Dict, _: bool = Depends(require_admin)):
    """Anzahl betroffener Einträge VOR dem Löschen (für den Bestätigungs-Dialog)."""
    rng, scope, sig_filter, trade_filter, _cutoff = _validate_clear_request(body)
    signals = await state.db.signals.count_documents(sig_filter)
    trades = await state.db.auto_trades.count_documents(trade_filter)
    return {"range": rng, "scope": scope, "signals": signals,
...
async def clear_analytics(body: Dict, _: bool = Depends(require_admin)):
    ...
    range: 'hour' | '24h' | '7d' | '4w' | 'all'
    scope: 'all' | 'coin' (nur symbol) | 'coin_strategy' (symbol + strategy_id)
           | 'strategy' (strategy_id über ALLE Coins)"""
    rng, scope, sig_filter, trade_filter, cutoff = _validate_clear_request(body)
