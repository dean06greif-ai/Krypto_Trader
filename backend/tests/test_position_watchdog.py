...
def test_watchdog_adopts_unknown_position_and_places_sl():
    ...
    # SL wurde an der Börse platziert
...
def test_watchdog_disabled_adoption_only_checks_sl():
    client = FakeClient(positions=[{"symbol": "ADAUSDT", "side": "BUY",
                                    "qty": "500", "avgOpenPrice": "0.5",
                                    "positionId": "p1"}])
    db = FakeDB()
    ...
    assert status["sl_fixed"] == 1  # SL wird trotzdem nachgezogen
