...
def test_regime_switch_is_detected_and_not_too_late():
    ...
    first_down = next((i for i, l in enumerate(after) if eng.split_id(l)[0] == 0), None)
...
def test_high_volatility_is_recognised():
    ...
    window = [eng.split_id(l)[1] for l in labels[310:380] if l is not None]
...
def test_validation_flags_wrong_labels():
    ...
    model, labels = labels_for(candles)
    wrong = [eng.regime_id(2, 1) if l is not None else None for l in labels]
...
def test_ideal_labels_agree_with_live_direction():
    ...
    agr = eng.agreement_with_ideal(labels, ideal)
...
def test_nnfx_mapping_covers_all_regimes():
    ...
    assert len(ids) == 9
    assert {eng.nnfx_regime(i) for i in ids} == {"trend", "range", "breakout"}
