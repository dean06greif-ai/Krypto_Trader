...
INDICATOR_ALIASES = {
    ...
    "ema9": "ema_fast", "ema_9": "ema_fast", "emafast": "ema_fast",
    "ema_short": "ema_fast", "ema21": "ema_fast",
    "ema50": "ema_slow", "ema_50": "ema_slow", "emaslow": "ema_slow",
    "ema_long": "ema_slow", "ema200": "ema_slow",
    ...
    for side in ("long_rules", "short_rules"):
        ...
        for i, r in enumerate(rules):
            if not isinstance(r, dict):
                ...
                continue
            rule = dict(r)
            ind = canonical_indicator(rule.get("indicator"))
            op = canonical_operator(rule.get("op"))
            rule["indicator"], rule["op"] = ind, op
            val = rule.get("value")
            if isinstance(val, str):
                cval = canonical_indicator(val)
                if cval in known_ind:
                    rule["value"] = cval
                else:
                    try:
                        rule["value"] = float(val)
                    except (TypeError, ValueError):
                        problems.append(f"{side}[{i}]: Wert '{val}' unbekannt")
            if ind not in known_ind:
                problems.append(f"{side}[{i}]: Indikator '{r.get('indicator')}' "
                                "wird nicht unterstützt")
            if op not in known_ops:
                problems.append(f"{side}[{i}]: Operator '{r.get('op')}' "
                                "wird nicht unterstützt")
...
# Binär (0/1) – nicht optimierbar
BINARY_INDICATORS = {"ha_color"}
...
def apply_params(definition: Dict, params: Dict) -> Dict:
    ...
    for side in ("long_rules", "short_rules"):
        ...
        for i, r in enumerate(rules):
            key = rule_param_key(side, i)
            if isinstance(r, dict) and key in params and params[key] is not None \
                    and isinstance(r.get("value"), (int, float)) \
                    and not isinstance(r.get("value"), bool):
                new_rules.append({**r, "value": params[key]})
            else:
                new_rules.append(r)
        out[side] = new_rules
    return out
