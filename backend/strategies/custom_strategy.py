...
class CustomStrategy(BaseStrategy):
    ...
    def _used_indicators(self, definition: Optional[Dict] = None):
        ...
        used = set()
        for r in (d.get("long_rules", []) + d.get("short_rules", [])):
    ...
    def _series(self, candles, definition: Optional[Dict] = None):
        d = (definition or self.definition).get("indicators", {})
        ...
        atr = (ind.calculate_atr(candles, p("atr_period", 14))
               if used & {"atr", "atr_pct", "keltner_upper", "keltner_lower"} else None)
        adx = pdi = mdi = cci = None
        kc_u = kc_m = kc_l = None
        dc_h = dc_l = None
        if used & {"adx", "plus_di", "minus_di", "cci", "keltner_upper",
                   "keltner_middle", "keltner_lower", "donchian_high", "donchian_low"}:
            from services import vec
            highs = np.array([c["high"] for c in candles], dtype=float)
            lows = np.array([c["low"] for c in candles], dtype=float)
            cl = np.array(closes, dtype=float)
            if used & {"adx", "plus_di", "minus_di"}:
                adx, pdi, mdi = vec.adx_di(highs, lows, cl, p("adx_period", 14))
            if "cci" in used:
                cci = vec.cci(highs, lows, cl, p("cci_period", 20))
            if used & {"keltner_upper", "keltner_middle", "keltner_lower"}:
                kc_m = vec.ema(cl, p("keltner_period", 20))
                mult = pf("keltner_mult", 2.0)
                atr_arr = np.array([np.nan if v is None else v for v in (atr or [])],
                                   dtype=float) if atr else np.full(len(candles), np.nan)
                kc_u = kc_m + mult * atr_arr
                kc_l = kc_m - mult * atr_arr
            if used & {"donchian_high", "donchian_low"}:
                dper = p("donchian_period", 20)
                dc_h, dc_l = [], []
                for i in range(len(candles)):
                    seg = candles[max(0, i - dper):i]
                    dc_h.append(max((c["high"] for c in seg), default=None))
                    dc_l.append(min((c["low"] for c in seg), default=None))
        ...
        def snap(i):
            ...
            if adx is not None:
                s["adx"] = _fin(adx[idx])
                s["plus_di"] = _fin(pdi[idx])
                s["minus_di"] = _fin(mdi[idx])
            if cci is not None:
                s["cci"] = _fin(cci[idx])
            if kc_m is not None:
                s["keltner_middle"] = _fin(kc_m[idx])
                s["keltner_upper"] = _fin(kc_u[idx])
                s["keltner_lower"] = _fin(kc_l[idx])
            if dc_h is not None:
                s["donchian_high"] = dc_h[idx]
                s["donchian_low"] = dc_l[idx]
    ...
    def analyze(self, candles: List[Dict], symbol: str, params: Dict) -> Optional[Dict]:
        # Optimierte Parameter (Perioden + Regel-Schwellen) werden auf die
        # Definition angewendet – identisch zum Fast-Path im Backtester.
