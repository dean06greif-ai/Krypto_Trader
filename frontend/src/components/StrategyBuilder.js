...
const StrategyBuilder = ({ strategies, enabledIds, onClose, onChanged }) => {
  ...
  const updRule = (list, setList, i, field, val) => {
    ...
    if (['indicator', 'op', 'value', 'valueType'].includes(field)) copy[i].label = '';
    setList(copy);
  };
  ...
  const deserializeRules = (list) => (list || []).map(r => {
    ...
    return { indicator: r.indicator, op: r.op, valueType: isInd ? 'indicator' : 'number', value: r.value, label: r.label || '' };
  });
