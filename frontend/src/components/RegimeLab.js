    ...
    <div data-testid="regime-analysis-detail">
      ...
      {isCombined
        ? (analysis.symbols || []).map(sym => (
          <RegimeChart key={sym} title={sym.replace('USDT', '')}
            ...
            liveSegments={showLive ? analysis.combined?.per_symbol?.[sym]?.live_segments : null}
        ...
        : (
          <RegimeChart title={symbol.replace('USDT', '')}
            ...
            liveSegments={showLive ? analysis.per_coin?.[symbol]?.live_segments : null}
      ...
      {Object.values(perSymbol).some(v => v?.live_segments?.length) && (
        <label className="opt-check" style={{ paddingBottom: 0 }}
          title="Band oben im Chart: so hat die Erkennung die Phasen in ECHTZEIT gesehen (ohne rückwirkende Korrektur bis zum Umkehrpunkt). Abweichungen zum Hintergrund = Erkennungsverzögerung an den Umkehrpunkten.">
          <input type="checkbox" checked={showLive} onChange={e => setShowLive(e.target.checked)}
            data-testid="regime-show-live" /> Live-Sicht einblenden (Erkennungsverzögerung)
        </label>
      )}
