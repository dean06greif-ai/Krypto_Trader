...
import { Robot, PaperPlaneRight, X, Trash, ArrowsClockwise, Lightning, CaretDown, CaretUp, Newspaper, PushPin, Brain, GraduationCap, CheckCircle, XCircle, Sliders, Coins, Flask, ArrowCounterClockwise, PencilSimple, Crown, Plus, FloppyDisk, Warning } from '@phosphor-icons/react';
...
const MODEL_OPTIONS = [
  ...
  { provider: 'openrouter', model: 'deepseek/deepseek-r1:free', label: 'OpenRouter · DeepSeek R1 (kostenlos, Top-Reasoning)' },
  { provider: 'openrouter', model: 'qwen/qwen3-235b-a22b:free', label: 'OpenRouter · Qwen3 235B (kostenlos, sehr stark)' },
              ...
              <h2>KI TRADER
                {((status?.providers_health?.rate_limited?.length || 0)
                  ...
                  <span
                    ...
                    title="Klicken für Details: welche KI-Modelle sind rate-limited oder ausgefallen"
                  >
                ...
                <div className="ai-health-dropdown" data-testid="ai-health-dropdown">
                  ...
                  {(status?.providers_health?.rate_limited || []).map(m => (
                    <div className="ai-health-dd-row limited" key={`rl-${m.provider}/${m.model}`}>
                      ...
                      <span>Rate-Limit – frei in ca. {Math.ceil((m.cooldown_left_s || 0) / 60)} min</span>
                    </div>
                  ))}
                  {(status?.providers_health?.errors || []).map(m => (
                    <div className="ai-health-dd-row err" key={`er-${m.provider}/${m.model}`}>
                      <span className="ai-health-dd-model">{m.provider}/{m.model}</span>
                      <span>{m.detail || 'Fehler/Timeout'}</span>
                    </div>
                  ))}
