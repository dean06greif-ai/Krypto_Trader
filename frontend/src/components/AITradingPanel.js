...
const MODEL_OPTIONS = [
  // Google Gemini (GEMINI_API_KEY) – Slugs live verifiziert (Modell-Wächter prüft wöchentlich)
  ...
  // OpenRouter (OPENROUTER_API_KEY) – Free-Katalog live verifiziert
            ...
            <div>
              <h2>KI TRADER
                {((status?.providers_health?.rate_limited?.length || 0)
                  ...
                  + (status?.providers_health?.skipped_too_large?.length || 0)) > 0 && (
                  <span
                    className="ai-health-badge clickable"
                    data-testid="ai-health-badge"
                    onClick={() => setShowHealth(v => !v)}
                    title="Klicken für Details: welche KI-Modelle sind rate-limited, ausgefallen oder wegen zu großem Prompt übersprungen"
                  >
