...
import { Robot, PaperPlaneRight, X, Trash, ArrowsClockwise, Lightning, CaretDown, CaretUp, Newspaper, PushPin, Brain, GraduationCap, CheckCircle, XCircle, Sliders, Coins, Flask, ArrowCounterClockwise, PencilSimple, Crown, Plus, FloppyDisk } from '@phosphor-icons/react';
...
const MODEL_OPTIONS = [
  ...
  // GitHub Models (GITHUB_MODELS_TOKEN) – kostenlose "Copilot"-Modelle per GitHub-Token
  { provider: 'github', model: 'openai/gpt-4.1', label: 'GitHub · GPT-4.1 (kostenlos, sehr stark)' },
  { provider: 'github', model: 'openai/gpt-4.1-mini', label: 'GitHub · GPT-4.1 Mini (kostenlos)' },
  { provider: 'github', model: 'openai/gpt-4o-mini', label: 'GitHub · GPT-4o Mini (kostenlos)' },
  // Cerebras (CEREBRAS_API_KEY) – free tier, extrem schnelle Inferenz
  { provider: 'cerebras', model: 'gpt-oss-120b', label: 'Cerebras · GPT-OSS 120B (kostenlos, stark)' },
  { provider: 'cerebras', model: 'llama-3.3-70b', label: 'Cerebras · Llama 3.3 70B (kostenlos)' },
  { provider: 'cerebras', model: 'qwen-3-32b', label: 'Cerebras · Qwen3 32B (kostenlos)' },
];
...
const AITradingPanel = ({ onClose, selectedCoin = 'BTCUSDT' }) => {
  ...
  const toggleLearn = () => {
    setShowLearn(prev => {
      const next = !prev;
      if (next) { loadInsights(); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleSetup = () => {
    setShowSetup(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleTeam = () => {
    setShowTeam(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowLab(false); setShowGov(false); setShowStrat(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleLab = () => {
    setShowLab(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowGov(false); setShowStrat(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleGov = () => {
    setShowGov(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowStrat(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleStrat = () => {
    setShowStrat(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); closeChatFocus(); }
      return next;
    });
  };
  ...
  // Schnellauswahl: eine ganze Anlageklasse (bzw. alle Assets) fokussieren.
  const selectGroup = (groupName) => {
    const g = (ASSET_GROUPS || []).find(x => x.name === groupName);
    const syms = (g?.symbols || []).map(s => s.symbol).filter(s => ALL_COINS.includes(s));
    if (syms.length) persistCoins(syms);
  };
  const groupSelected = (groupName) => {
    const g = (ASSET_GROUPS || []).find(x => x.name === groupName);
    const syms = (g?.symbols || []).map(s => s.symbol);
    return !allSelected && syms.length > 0 && syms.length === chatCoins.length
      && syms.every(s => chatCoins.includes(s));
  };
  ...
  return (
    <div className="ai-panel-overlay" onClick={onClose} data-testid="ai-trading-panel">
      <div className="ai-panel" onClick={e => e.stopPropagation()}>
              ...
              return (
                <div key={rd.key} className={`ai-role-card ${rc.enabled === false ? 'disabled' : ''}`} data-testid={`ai-role-card-${rd.key}`}>
                  ...
                  <div className="ai-role-grid">
                    ...
                    <label title="Springt ein, wenn das Rollen-Modell außerhalb der Handelszeiten ist oder komplett scheitert">
                      <span>Fallback-KI</span>
          ...
          <div className="ai-decisions-wrap" data-testid="ai-coin-selector">
            <div className="ai-chat-focus-bar">
              ...
              <div className="ai-focus-quick" data-testid="ai-focus-quick">
                ...
                {FOCUS_GROUPS.filter(g => (ASSET_GROUPS || []).some(x => x.name === g.group))
                  .map(g => (
                    <button
                      key={g.group}
                      className={`ai-coin-all-toggle ${groupSelected(g.group) ? 'active' : ''}`}
                      onClick={() => selectGroup(g.group)}
                      title={`Nur ${g.label} fokussieren`}
                      data-testid={`ai-focus-group-${g.group.toLowerCase().replace(/\s+/g, '-')}`}
                    >
                      {g.label}
                    </button>
                  ))}
        ...
        {/* Chat + Input – in den Vollbild-Tabs (Lernen, Labor, MasterPrompt,
            Strategien, KI-Team) komplett ausgeblendet */}
        {!showLearn && !showLab && !showGov && !showStrat && !showTeam && (
        <>
