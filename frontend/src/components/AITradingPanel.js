...
import { Robot, PaperPlaneRight, X, Trash, ArrowsClockwise, Lightning, CaretDown, CaretUp, Newspaper, PushPin, Brain, GraduationCap, CheckCircle, XCircle, Sliders, Coins, Flask, ArrowCounterClockwise, PencilSimple, Crown, Plus, FloppyDisk, Warning, ArrowDown, Prohibit } from '@phosphor-icons/react';
...
const AITradingPanel = ({ onClose, selectedCoin = 'BTCUSDT' }) => {
  ...
  const chatVisible = !showLearn && !showLab && !showGov && !showStrat && !showTeam;
  useEffect(() => {
                  ...
                  >
                    ...
                    {String(d?.action).toUpperCase() === 'HOLD' ? (
                      <span className="ai-dec-noedge" data-testid={`ai-dec-noedge-${d?.symbol || i}`}
                        title="Bewusst KEIN Trade – die KI sieht aktuell keinen klaren Edge (z.B. schlechtes Handelsfenster oder Trade-Sperre bis klare Struktur). Kein Fehler – Grund steht in der Begründung.">
      ...
      <div className="ai-panel" onClick={e => e.stopPropagation()}>
        ...
        {showSetup && (
          <div className="ai-setup" data-testid="ai-setup-panel">
            ...
            {tokenUsage?.days?.length > 0 && (
              <div style={{ gridColumn: '1 / -1' }} data-testid="ai-token-usage">
