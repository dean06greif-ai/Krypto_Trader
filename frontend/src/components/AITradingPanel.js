...
const AITradingPanel = ({ onClose, selectedCoin = 'BTCUSDT' }) => {
  ...
  const loadProposals = useCallback(async () => {
    try {
      // Autonomie "auto": nur echte pending-Vorschläge zeigen. Geparkte Wünsche
      // (needs_data/needs_confirmation) verwaltet die KI dann selbst und wendet
      // sie automatisch an, sobald die Daten reichen – kein Hinweis-Spam.
      const auto = statusRef.current?.config?.autonomy === 'auto';
      const states = auto ? [] : ['pending', 'needs_confirmation', 'needs_data'];
      const lists = await Promise.all(states.map(
  ...
  return (
    <div className="ai-panel-overlay" onClick={onClose} data-testid="ai-trading-panel">
      <div className="ai-panel" onClick={e => e.stopPropagation()}>
        ...
        {proposals.length > 0 && !showLab && !showGov && !showStrat && (
        ...
        {!showLearn && !showLab && !showGov && !showStrat && (
        <>
