...
const TradeDetailCard = ({ t, stratName, getCoinName, onChanged }) => {
  ...
  <span className="tdc-main">
    ...
    {t.horizon === 'swing' && <span className="badge badge-swing" data-testid={`trade-swing-badge-${t.id}`}>SWING{t.runner ? ' · RUNNER' : ''}</span>}
        ...
        <div className="tdc-body" data-testid={`trade-card-body-${t.id}`}>
          ...
          <div className="tdc-meta">
            ...
            {!closed && c.upnl_pct_margin != null && (
              <div className="tdc-meta-item"><span>uPnL % auf Margin (wie Bitunix)</span><b className={`mono ${(c.upnl_pct_margin || 0) >= 0 ? 'text-long' : 'text-short'}`} data-testid={`trade-upnl-pct-margin-${t.id}`}>{fmtPct(c.upnl_pct_margin)}</b></div>
            )}
            <div className="tdc-meta-item"><span>PnL % auf Margin</span><b className={`mono ${(c.pnl_pct_margin || 0) >= 0 ? 'text-long' : 'text-short'}`} data-testid={`trade-pnl-pct-margin-${t.id}`}>{c.pnl_pct_margin != null ? fmtPct(c.pnl_pct_margin) : '—'}</b></div>
            <div className="tdc-meta-item"><span>PnL % Positionsgröße</span><b className={`mono ${(c.pnl_pct || 0) >= 0 ? 'text-long' : 'text-short'}`} data-testid={`trade-meta-pnl-pct-${t.id}`}>{c.pnl_pct != null ? fmtPct(c.pnl_pct) : '—'}</b></div>
