        ...
        <div className="tdc-body" data-testid={`trade-card-body-${t.id}`}>
          ...
          <div className="tdc-meta">
            ...
            {c.price_move_pct != null && (
              <div className="tdc-meta-item" title="Reine Kursbewegung seit Entry in Trade-Richtung – ohne Hebel, ohne Gebühren. Positiv = der Kurs läuft für dich."><span>Kursgewinn (reine Bewegung)</span><b className={`mono ${(c.price_move_pct || 0) >= 0 ? 'text-long' : 'text-short'}`} data-testid={`trade-price-move-pct-${t.id}`}>{fmtPct(c.price_move_pct)}</b></div>
            )}
